...
class Habito:
    ...
    def inserirHabito(self, email):
        ...
        try:
            data_inicial = input('Data de início (DD/MM/AAAA): ').strip()
            datetime.strptime(data_inicial, '%d/%m/%Y')
            data_final = input('Data de término (DD/MM/AAAA): ').strip()
        ...
        self.db.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ''', (email, nome, date.today(), data_inicial, data_final, frequencia, motivacao))
    ...
    def progresso(self):
        ...
        for habito in habitos:
            ...
            feitos = self.db.execute("SELECT COUNT(*) FROM habito_progresso WHERE id_habito = ?", (id_habito,)).fetchone()[0]
            ...
            if input("Marcar progresso? (s/n): ").strip().lower() == 's':
                data = input("Data (DD/MM/AAAA), ou Enter para hoje: ").strip() or hoje.isoformat()
                status = input("Status (Feito/Não feito): ").strip().capitalize()

                try:
                    datetime.strptime(data, '%d/%m/%Y')
                    self.db.execute('''INSERT INTO habito_progresso (id_habito, data, status) VALUES (?, ?, ?)''',
                                    (id_habito, data, status))
